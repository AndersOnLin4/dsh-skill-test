...
description: 主 Agent 任务分配与子 Agent 精简返回模板。当需要把独立子任务（搜索、长文档抽取、分类、摘要、格式转换、多模态识别与生成）外包以节省主上下文 token 时使用。提供外包决策规则、子 AI 通道选择（豆包/DeepSeek/普通子 Agent）、转述不设计铁律、delegate_task 工具定义、12 套子 Agent 强制返回 Schema，确保子 Agent 只返回结构化 JSON，不返回原始数据和过程废话。多模态任务必须外包，主 Agent 通过子 Agent 返回的结构化描述实现伪多模态化。完整 AI 能力路由表见 references/routing.md——按需加载，勿让整表常驻上下文。
version: 3.0.0
---
...
|-------|------------------------|------|
| 豆包（本机桌面端） | `doubao` | 创作/多模态/办公（详见 references/routing.md） |
...
> 完整路由表在 `references/routing.md`，**按需加载**——除非要精确查表，否则只读本节要点，别把整表读进上下文。

分派前先识别任务**能力 ID**，再从市面最强 AI 起逐个检查"当前环境有没有对应 skill"，命中第一个可用的就用；都不存在才降级（普通子 Agent → 主 Agent 自己），交付时说明降级原因。要点：

- **环境自适应**：路由只认当前环境实际存在的 skill——装了 `gpt` 驱动就自动升 GPT，缺了就顺延下一名。路由表零改动、skill 不需要能力声明字段。
- **本机现状**：只有 `doubao`、`deepseek-web`、`github-push` 三个通道 skill，除 coding/reasoning 路由到 DeepSeek 外，其余能力实际都顺延到豆包。
- **UI 通道健壮性**：`doubao` / `deepseek-web` 内置窗口就位保障 + 探测工具。报错（`DOUBAO_ERROR:` / `DEEPSEEK_WEB_ERROR:`）且重试一次仍失败时，按「锚点→候选→验证→降级」方法链用 probe 工具排障（见两个通道 SKILL.md 的「UI 发现方法链」铁律），走完方法链前禁止硬点坐标。
- **生成类成品取回**：按通道 skill 的取回流程拿文件（豆包：`-Action extract` / `download-asset.ps1`）。
...
## 委派的两种形态（关键区分！写 skill 最容易产生歧义的地方，务必遵守）
