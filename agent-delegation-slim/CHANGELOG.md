...
- 目录更名：`agent-delegation-slim-v2` → `agent-delegation-slim`（唯一主版本）；v1 原版归档至 `G:\harness\dsh-home\skills-archive\agent-delegation-slim-v1\`。
- 路由表不再写"优先 v2、回退 v1"：通道 skill 统一无后缀主名（`doubao` / `deepseek-web`）。
- **瘦身**：完整「AI 能力路由」表（优先级快照 + 路由算法 + 命名约定 + UI 通道方法链）从 SKILL.md 抽离至 `references/routing.md`，SKILL.md 只保留使用要点与指针——路由表按需加载，不再随 SKILL.md 常驻上下文。
- 动因：实测单次识图调度消耗 210K 输入 token，SKILL.md 全文 + 路由表是固定开销大头（详见 `G:\文档\网络学习笔记\token消耗过大.md`）。
